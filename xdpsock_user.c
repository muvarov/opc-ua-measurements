...
ssize_t af_xdp_recvfrom(void *buf)
{
	struct xdp_desc descs[1];
	unsigned int rcvd, i;
	struct xdpsock *xsk = xsks[0];

	rcvd = xq_deq(&xsk->rx, descs, 1);
	if (!rcvd)
		return 0;
...
static int xdp_init(void)
{
	...
	iface = getenv("XDP_IFACE");
	if (!iface) {
		fprintf(stderr, "export XDP_IFACE=eth0 for your interface is needed. quit.\n");
		exit(EXIT_FAILURE);
	}

	opt_ifindex = if_nametoindex(iface);
	printf("%s() for interface %s\n", __func__, iface);

	snprintf(xdp_filename, sizeof(xdp_filename), "%s_kern.o", "xdpsock");
